...
def load_rows_list(dataset_dir,field):
	if type(dataset_dir)!=str or len(dataset_dir)<=1 or type(field)!=str or len(field)<=1:
		raise Exception("[create_pddlinputs_pour_exp4.py] Wrong input.");
	...
	if len(text)<=1 or type(text)!=list:
		raise Exception("[create_pddlinputs_pour_exp4.py] Wrong computation.");
...
def main():
	...
	dataset_dir = 'C:/Users/David/OneDrive - National University of Singapore/PhD_work/Code/TAMP_HighLevel/Visual_Studio/taskplanningmodule/Language_Model_many2many_novelty/input_comparisons/input_pour_Exp4_knownTasks.csv';
	updated_dataset_dir = 'C:/Users/David/OneDrive - National University of Singapore/PhD_work/Code/TAMP_HighLevel/Visual_Studio/taskplanningmodule/Language_Model_many2many_novelty/input_comparisons/input_pour_Exp4_knownTasks_updated.csv';
	...
	for k in range(0,len(our_goals_list)):
		...
		loc1 = our_goal[3];# Location of the first object
		loc2 = our_goal[6];# :Location of the second object
		...
		pddl_goal[3] = obj1;
		pddl_goal[4] = str(loc1);
		pddl_goal[5] = obj2;
		pddl_goal[6] = str(loc2)+")))) \\n";
