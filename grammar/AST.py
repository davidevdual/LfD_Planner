...
class Node:
    ...
    def thread_ast(self,lastNode):
        #print("lastNode = ",lastNode," children = ",lastNode.children);
        for c in self.children:
            lastNode = c.thread_ast(lastNode);
        lastNode.addNext(self);
        #print("lastNode.next = ",lastNode.next);
        return self;
        
class PlanNode(Node):
    type = 'START';
    def __init__(self, last_action, children):
...
# Node for conjunction
class ConjNode(Node):
    type = 'conj';
    def __init__(self, conj, children):
        Node.__init__(self,children);
        self.conj = conj;
        try:
            self.nbargs = len(children);
...
class ActionNode(Node):
    ...
    def __repr__(self):
        return "%s (%s)" % (self.action, self.nbargs);
