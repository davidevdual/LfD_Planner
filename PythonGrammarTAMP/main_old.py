...
import Motion_Planning.motionplanning as mp
...
def initialize_robot(bodyUniqueId,sim,viewer,endEffectorLinkIndex_left,endEffectorLinkIndex_right):
	...
	orientation_left = [[0, 0, 0, 1]]# No rotation
	pose_right = [[0.5149956941604614, -0.30806973576545715, 1.978783011436462]]
	orientation_right = [[0, 0, 0, 1]]# No rotation
	...
	if initialized == False:
		...
		#ls = p.getLinkState(bodyUniqueId,endEffectorLinkIndex_left)
		#print("The actual pose is: ",ls[4],ls[5])
...
def fill_matrix(bigger_size,smaller_size):
	...
	#print("The last_row is: ", last_row)
...
def pad(input_1,input_2):
	#print("The last row of input_2 is: ", input_2[-1])
	#print("The last row of input_1 is: ", input_1[-1])
...
def oversample(states, nb_samples):
	...
	for k in range(0,len(states)):
		...
		#print(np.linspace(state_positions[0], state_positions[1], num=50, axis=0))

	oversampled_states = np.delete(oversampled_states, (0), axis=0)
	#print(oversampled_states)
	return oversampled_states

def extract_pose_trajectories(geometric_path):
	#print("The number of states is: ",geometric_path.getStateCount())
	state_positions = np.zeros(3)
	state_orientations = np.zeros(4)

	#print("The state positions before append are: ",state_positions)
	#print("The state orientations before append are: ",state_orientations)
	...
	print("state_positions sampling: ",state_positions.shape)

	#print("The state positions are: ",state_positions)
	#print("The state orientations are: ",state_orientations)
...
def get_urdf_data(urdf_path):
	...
	#print("The orientation is: ",robotStartOrientation)
...
def get_positions_eef(bodyUniqueId,endEffectorLinkIndex,trajectory_positions,trajectory_orientations):
	...
	#print("Number of joints: ",p.getNumJoints(bodyUniqueId)) 
	for k in range(0,len(trajectory_positions)):   
		#print("Trajectory positions: ",trajectory_positions[k])
...
def compute_motion_plan(task_plan,bodyUniqueId,endEffectorLinkIndex_left,endEffectorLinkIndex_right,init_position_left,init_position_right,init_orientation_left,init_orientation_right):
	motion_planner = mp.MotionPlanner(bodyUniqueId,endEffectorLinkIndex_left,endEffectorLinkIndex_right,init_position_left,init_position_right,init_orientation_left,init_orientation_right)
	motion_plan = motion_planner.compute_plan(task_plan)
	return False

def compute_joint_angles(bodyUniqueId, endEffectorLinkIndex, targetPosition, targetOrientation):
	ikSolver = 0
	joint_angles = p.calculateInverseKinematics(bodyUniqueId,endEffectorLinkIndex,targetPosition,
...
if __name__ == "__main__":
	...
	model = load_model_from_path("../xmls/robotic_arm_simulator.xml")
	sim = MjSim(model)
	viewer = MjViewer(sim)
	...
	print(task_plan)
	motion_plan = compute_motion_plan(task_plan,bodyUniqueId,endEffectorLinkIndex_left,endEffectorLinkIndex_right,
	...
	'''
	bodyUniqueId,endEffectorLinkIndex_left,endEffectorLinkIndex_right = get_urdf_data(urdf_path)
	#print("init_position_left: ",init_position_left)
	#print("init_position_right: ",init_position_right)
	#print("init_position_left_begin,init_orientation_left_begin: ",init_position_left,init_orientation_left)
	initialize_robot(bodyUniqueId,sim,viewer,endEffectorLinkIndex_left,endEffectorLinkIndex_right)
	init_position_left,init_orientation_left = get_ee_pose(bodyUniqueId,endEffectorLinkIndex_left)
	init_position_right,init_orientation_right = get_ee_pose(bodyUniqueId,endEffectorLinkIndex_right)
	#print("init_position_left: ",init_position_left)
	...
	targetPosition_left = [0.9, 0.05, 1.92]
	targetOrientation_left = [0, 0, 0, 1]# No rotation
	targetPosition_right = [0.65, -0.30806973576545715, 2.05]
	targetOrientation_right = [0, 0, 0, 1]# No rotation
	#print("init_position_left,init_orientation_left: ", init_position_left,init_orientation_left)

	#results_time = []
	#for k in range(0,100):
	#	start = time.time()
	geometric_path_left = rigid_body_planning.compute_plan(1, init_position_left, targetPosition_left, init_orientation_left, targetOrientation_left)
	geometric_path_right = rigid_body_planning.compute_plan(1, init_position_right, targetPosition_right, init_orientation_right, targetOrientation_right)
	#	end = time.time()
	#	results_time.append(end - start)
	#results_time_matrix = np.asarray(results_time)
	#print(results_time_matrix)
	#np.save('results_classical_motion_planner_withConstraints.npy',results_time_matrix)
	#print("The mean is: ", np.mean(results_time_matrix))
	#print("The standard deviation is: ",np.std(results_time_matrix))
	#print("--- %s seconds ---" % (time.time() - start_time))

	trajectory_positions_left,trajectory_orientations_left = extract_pose_trajectories(geometric_path_left)
	trajectory_positions_right,trajectory_orientations_right = extract_pose_trajectories(geometric_path_right)

	print("Shape of trajectory_positions_left: ",trajectory_positions_left.shape)
	print("Shape of trajectory_positions_right: ",trajectory_positions_right.shape)
	leftEEF_positions = get_positions_eef(bodyUniqueId,endEffectorLinkIndex_left,trajectory_positions_left,trajectory_orientations_left)
	rightEEF_positions = get_positions_eef(bodyUniqueId,endEffectorLinkIndex_right,trajectory_positions_right,trajectory_orientations_right)
	leftEEF_positions,rightEEF_positions = pad(leftEEF_positions,rightEEF_positions)
	...
	numJoints = p.getNumJoints(bodyUniqueId)
	ikSolver = 0
	#print("Number of rows leftEEF_positions: ",len(leftEEF_positions))
	#print("Number of rows rightEEF_positions: ",len(rightEEF_positions))

	desired_left_trajectories = np.zeros(trajectory_positions_right.shape[1])
	desired_right_trajectories = np.zeros(trajectory_positions_right.shape[1])
	measured_left_trajectories = np.zeros(trajectory_positions_right.shape[1])
	measured_right_trajectories = np.zeros(trajectory_positions_right.shape[1])

	print("Wait for execution")
	while True:
		if executed == False and time>time_max:
			for k in range(0,len(leftEEF_positions)):
				print("The pose for left_ee is: ", get_ee_pose(bodyUniqueId,endEffectorLinkIndex_left))
				print("The pose for right_ee is: ", get_ee_pose(bodyUniqueId,endEffectorLinkIndex_right))
				measured_pose_left = get_ee_pose(bodyUniqueId,endEffectorLinkIndex_left)[0]
				measured_pose_right = get_ee_pose(bodyUniqueId,endEffectorLinkIndex_right)[0]
				
				sim_state = sim.get_state()
				leftEEF_position = leftEEF_positions[k]
				rightEEF_position = rightEEF_positions[k]
				
				sim_state = sim.get_state()
				sim_state.qpos[sim.model.get_joint_qpos_addr("LJ_1")] = leftEEF_position[0]
				sim_state.qpos[sim.model.get_joint_qpos_addr("LJ_2")] = leftEEF_position[1]
				sim_state.qpos[sim.model.get_joint_qpos_addr("LJ_3")] = leftEEF_position[2]
				sim_state.qpos[sim.model.get_joint_qpos_addr("LJ_4")] = leftEEF_position[3]
				sim_state.qpos[sim.model.get_joint_qpos_addr("LJ_5")] = leftEEF_position[4]
				sim_state.qpos[sim.model.get_joint_qpos_addr("LJ_6")] = leftEEF_position[5]
				sim_state.qpos[sim.model.get_joint_qpos_addr("RJ_1")] = rightEEF_position[23]
				sim_state.qpos[sim.model.get_joint_qpos_addr("RJ_2")] = rightEEF_position[24]
				sim_state.qpos[sim.model.get_joint_qpos_addr("RJ_3")] = rightEEF_position[25]
				sim_state.qpos[sim.model.get_joint_qpos_addr("RJ_4")] = rightEEF_position[26]
				sim_state.qpos[sim.model.get_joint_qpos_addr("RJ_5")] = rightEEF_position[27]
				sim_state.qpos[sim.model.get_joint_qpos_addr("RJ_6")] = rightEEF_position[28]

				p.resetJointState(bodyUniqueId,1,leftEEF_position[0])
				p.resetJointState(bodyUniqueId,2,leftEEF_position[1])
				p.resetJointState(bodyUniqueId,3,leftEEF_position[2])
				p.resetJointState(bodyUniqueId,4,leftEEF_position[3])
				p.resetJointState(bodyUniqueId,5,leftEEF_position[4])
				p.resetJointState(bodyUniqueId,6,leftEEF_position[5])
				p.resetJointState(bodyUniqueId,24,rightEEF_position[23])
				p.resetJointState(bodyUniqueId,25,rightEEF_position[24])
				p.resetJointState(bodyUniqueId,26,rightEEF_position[25])
				p.resetJointState(bodyUniqueId,27,rightEEF_position[26])
				p.resetJointState(bodyUniqueId,28,rightEEF_position[27])
				p.resetJointState(bodyUniqueId,29,rightEEF_position[28])
				...
				#print("measured_left_trajectories shape: ",measured_left_trajectories.shape)
				#print("measured_pose_left shape: ",measured_pose_left.shape)
				measured_left_trajectories = np.vstack((measured_left_trajectories,measured_pose_left))
				measured_right_trajectories = np.vstack((measured_right_trajectories,measured_pose_right))
				
				if os.getenv('TESTING') is not None:
					break
				if k==len(leftEEF_positions)-1:
					executed = True
			if executed == True:
				break
	...
	measured_left_trajectories = np.delete(measured_left_trajectories, (0), axis=0)
	measured_right_trajectories = np.delete(measured_right_trajectories, (0), axis=0)

	plotting = plot.Plotting()
	plotting.plot_desired_measured(trajectory_positions_left,measured_left_trajectories)
	plotting.plot_desired_measured(trajectory_positions_right,measured_right_trajectories)
