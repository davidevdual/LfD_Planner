...
def compute_task_plan():
	task_planner = human_tamp.HumanTAMP("/home/yulab/Documents/PhD_Work/mujoco_dualarm_simulation/trained_models/model_TAMP_22_02_2020.pth")
		...
		if executed == False and time>time_max:
			...
			for k in range(0,len(motion_plan_left)):
				...
				if k==len(motion_plan_left)-1:
					sim_state.qpos[sim.model.get_joint_qpos_addr("RJ_5")] = -1.57
					p.resetJointState(bodyUniqueId,28,rightEEF_position[27])
					sim.set_state(sim_state)
					sim.forward()
					viewer.render()
